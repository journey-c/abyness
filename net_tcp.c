...
static tcp_listener *_net_tcp_server(char *err, char *bind_addr, int port,
    ...
    if ((rc = getaddrinfo(bind_addr, _port, &hints, &listp)) != 0) {
        net_set_error(err, "[_net_tcp_server getaddrinfo] %s",
                      gai_strerror(rc));
        return NULL;
    }
    for (p = listp; p; p = listp->ai_next) {
        if ((tl->fd = socket(p->ai_family, p->ai_socktype, p->ai_protocol)) ==
            -1) {
            continue;
        }
...
tcp_conn *net_tcp_accept(char *err, tcp_listener *tl) {
    ...
    if (fd == NET_ERR) {
        net_set_error(err, "[net_tcp_accept accept] %d %s", errno,
                      strerror(errno));
        return NULL;
    }
